...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-12">
      ...
      {article.category && <span className="text-xs font-semibold tracking-widest text-[#0d4a2a] uppercase block mb-2">{article.category}</span>}
      ...
      <div className="mt-8 max-w-none text-gray-700 leading-relaxed space-y-5">
        {article.content.split('\n').filter((p: string) => p.trim()).map((p: string, i: number) => {
          ...
          if (/^\d+\.\s/.test(p)) return <p key={i} className="pl-4 border-l-2 border-[#2db87d] text-gray-700"><strong>{p.match(/^\d+\./)?.[0]}</strong> {p.replace(/^\d+\.\s/, '')}</p>;
