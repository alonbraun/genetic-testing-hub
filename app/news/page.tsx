...
export const metadata = {
  title: "Genetic Testing News & Insights — GeneticTesting.com",
  description: "The latest news, funding rounds, and insights from the genetic testing and genomics industry.",
};

const CATEGORY_COLORS: Record<string, string> = { Funding: "#d4f0e2", Product: "#d4ede0", Research: "#d0f0e8", Policy: "#fef3d0", Industry: "#dff5ea" };
const CATEGORY_TEXT: Record<string, string> = { Funding: "#0d4a2a", Product: "#0a3d22", Research: "#0a4a35", Policy: "#a06b00", Industry: "#0d5c32" };
    ...
    <>
      <section className="bg-[#0a2818] px-5 py-16 md:py-20">
        <div className="max-w-7xl mx-auto">
          <span className="text-[#2db87d] text-xs font-semibold tracking-[3px] uppercase">News & Insights</span>
          <h1 className="text-4xl md:text-5xl font-semibold text-white mt-4 mb-3 leading-tight">Genetic testing industry news</h1>
          <p className="text-[#86d4a8] text-lg max-w-2xl">Funding rounds, product launches, research findings, and policy updates — published 3× per week.</p>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-5 py-12">
        {articles.length > 0 ? (
          <>
            {lead && (
              <Link href={`/news/${lead.slug}`} className="group block mb-12 rounded-3xl overflow-hidden border border-gray-100 hover:border-[#0d4a2a]/20 hover:shadow-lg transition-all">
                ...
                <div className="p-8 bg-white">
                  <h2 className="text-2xl md:text-3xl font-semibold text-gray-900 leading-snug group-hover:text-[#0d4a2a] transition-colors">{lead.title}</h2>
            ...
            {rest.length > 0 && (
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
                {rest.map((a) => (
                  <Link key={a.slug} href={`/news/${a.slug}`} className="group bg-white rounded-2xl overflow-hidden border border-gray-100 hover:border-[#0d4a2a]/20 hover:-translate-y-0.5 hover:shadow-md transition-all">
                    ...
                    <div className="p-5">
                      <h3 className="mt-2 text-sm font-semibold text-gray-900 leading-snug line-clamp-3 group-hover:text-[#0d4a2a] transition-colors">{a.title}</h3>
          ...
          <div className="text-center py-28">
            ...
            <Link href="/newsletter" className="bg-[#0d4a2a] text-white px-6 py-3 rounded-xl font-medium hover:bg-[#082d19] transition-colors">Subscribe</Link>
          </div>
        )}

        <div className="mt-16 bg-[#0a2818] rounded-3xl p-8 md:p-10 flex flex-col md:flex-row items-center gap-6">
          <div className="flex-1">
            <p className="text-[#2db87d] text-xs font-bold tracking-widest uppercase mb-2">Sponsored content</p>
            <h3 className="text-xl font-semibold text-white mb-2">Reach genetic testing decision-makers with your story</h3>
            <p className="text-[#86d4a8] text-sm">Sponsored articles appear in the News feed and the weekly newsletter. $500/article.</p>
          </div>
          <Link href="/advertise" className="shrink-0 bg-[#0d4a2a] hover:bg-[#0f5c32] text-white font-medium px-6 py-3 rounded-xl transition-colors">Learn more →</Link>
        </div>
      </div>
    </>
  );
