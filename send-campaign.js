...
async function publishCompany(lead) {
  ...
  const data = await fetchJson(
    ...
    },
    {
      ...
      messages: [{
        role: "user",
        content: `Write a 2-sentence factual company description for a genetic testing industry directory listing.
Company: ${lead.company}
Website: ${website}
Contact title: ${lead.title}
Keywords: ${lead.relevance ? lead.relevance.split(",").slice(0, 8).join(", ") : "genetic testing"}
...
`;
  ...
  const filePath = `content/companies/${slug}.md`;
  ...
  try {
    const existing = await fetchJson(
      ...
      { headers: { Authorization: `token ${GITHUB_TOKEN}`, "User-Agent": "genetictesting/1.0" } }
    );
